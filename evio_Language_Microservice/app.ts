...
import languageV2Router from './controllers/language-v2.controller';
...
const port = Constants.NODE_ENV === 'production' ? process.env.PORT : process.env.PORT_DEV;

switch (Constants.NODE_ENV) {
  case 'production':
...
async function connectionDB() {
  const connString = String(process.env.DB_URI).replace('{database}', 'languageDB');
  await mongoose.connect(connString, options)
    .then((connection: { connections: { name: any; }[]; }) => {
...
app.use(require('./controllers/countryCode'));
app.use(languageV2Router);
