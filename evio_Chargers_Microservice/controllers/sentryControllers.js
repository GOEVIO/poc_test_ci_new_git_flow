...
module.exports = {
    samplingRateValidation: async function (samplingContext) {
        ...
        try {
            ...
            const rating = await getRating(allValidations , samplingContext)
    ...
    try {
        for (let validationObj of allValidations) {
            if (validationObj.validationType === process.env.sentryNameValidation) {
                ...
                const foundValidation = validationObj.validations.find(validation => filterValidation(transactionName , validation))
...
function filterValidation(element , validation) {
    ...
    try {
        if (validation.type === process.env.sentryEqualsValidation) return element === validation.value
        else if (validation.type === process.env.sentryIncludesValidation) return element?.includes(validation.value)
        return false
    } catch (error) {
        console.error(`[${context}][find] Error `, error.message);
        return null
