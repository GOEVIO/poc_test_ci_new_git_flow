...
describe('ChargerImagesController - listChargerImages', () => {
    ...
    test('should return charger images successfully', async () => {
        ...
        const spy = jest.spyOn(chargerImagesService, 'getChargerImages').mockResolvedValue(fakeResponse);

        await listChargerImages(req as Request, res as Response);

        expect(spy).toHaveBeenCalledWith('charger1');
    ...
    test('should handle errors', async () => {
        const spy = jest
            .spyOn(chargerImagesService, 'getChargerImages')
