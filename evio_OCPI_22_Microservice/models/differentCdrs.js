...
var DifferentCdrs = module.exports = mongoose.model('differentcdrs', cdrsModel);


//CDRs.ensureIndexes({ id: 1 }, (err) => {
DifferentCdrs.createIndexes({ id: 1 }, (err) => {
    if (err)
        console.error(err);
    else
...
module.exports.updateCDR = function (query, values, callback) {
    DifferentCdrs.findOneAndUpdate(query, values, callback);
