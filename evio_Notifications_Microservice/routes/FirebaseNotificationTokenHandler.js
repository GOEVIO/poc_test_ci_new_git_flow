...
const { ClientName } = require('../utils/constants');
                                        ...
                                        if (activeSubs.length !== 0) {
                                            ...
                                            var handle = new Promise((resolve, reject) => {
                                                activeSubs.forEach((topic, index, array) => {
                                                    firebaseConnect(ClientName).then((admin) => {
                                                        admin.
                                                            messaging().
...
const removePublicUserToken = ((res, newFirebaseToken) => {

    let firebaseToken = new FirebasePublicUserToken(newFirebaseToken);

    FirebasePublicUserToken.removeFirebaseTokenUser(firebaseToken, (error, result) => {
        if (error) {
            return res.status(400).send({ code: 'token_update_error', message: "Token update error" });
        }
        else {
