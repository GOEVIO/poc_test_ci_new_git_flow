...
module.exports = {
    handle: function (req, res, next, CentralSystemServer) {
        ...
        if (!connectorId)
            return res.status(400).send({ auth: false, code: "server_connector_id_required", message: 'Connector Id required' });
            ...
            if (charger) {
                ...
                Utils.getClient(context, charger.data.charger[0], CentralSystemServer).then((client) => {
                    ...
                    if (client) {
                        ...
                        console.log(`${context}Trying  unlock Connector ${connectorId} : ChargerId: ${hwId}; Endpoint: ${charger.data.charger[0].endpoint} `);

                        var data = new Object
                        data.connectorId = connectorId;

                        CentralSystemServer.unlockConnector(hwId, charger.data.charger[0].endpoint, data).then(function (result) {
                            ...
                            if (result) {
                                if (parser.validate(result.envelope) === true) {
                                    ...
                                    try {
                                        ...
                                        remoteStatus = jsonObj.Envelope.Body.unlockConnectorResponse.status;
                                ...
                                if (remoteStatus === 'Accepted') {
                                    ...
                                    return res.status(400).send({ auth: false, code: "server_ocpp_unlock_connector_not_accepted", message: 'Unlock Connector not accepted' });
                                }
                            ...
                            console.log(`${context} Error unlocking connector on charger ${hwId} with error: ${err} `)
