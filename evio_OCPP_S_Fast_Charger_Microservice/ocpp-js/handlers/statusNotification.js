  ...
  handle: function (data) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      try {
        ...
        updateChargingSessionToRunning(data);
...
//Function to stop transaction notification when user does not connects the plug
const updateChargingSessionToRunning = (data) => {

  //Check if connector id is available
  if (data.status == global.chargePointStatusOCPPOccupied) {
    ...
    Utils.checkIfHasChargingSession(chargingSessionServiceProxy, params).then((chargingSession) => {
      ...
      if (chargingSession) {
        //Update charging Session with failed
        updateChargingSession(chargingSessionServiceProxy, process.env.SessionStatusRunning, chargingSession);
        console.log(`[StatusNotification] A charging session just started: ${data.chargeBoxIdentity} and connectorId  ${data.connectorId}`)
      }
