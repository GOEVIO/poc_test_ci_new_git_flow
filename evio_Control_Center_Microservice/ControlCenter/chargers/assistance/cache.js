...
module.exports = {
    send: (req,res) => sendClearCache(req,res)
};

async function sendClearCache(req,res) {
    let context = "Function sendClearCache";
    try {
        if (validateFields(req.body)) return res.status(400).send(validateFields(req.body))

        let host = process.env.HostOCPP16 + process.env.PathSendClearCache
