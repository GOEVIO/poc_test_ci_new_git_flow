...
@Injectable()
export class PrivateSessionsClient implements GetSessionClientInterface {
  ...
  > {
    try {
      const baseURL = this.configService.get('client.charger.host')
      if (!baseURL) {
        throw new InternalServerErrorException(
          'CHARGER_SERVICE_HOST environment variable is not set'
        )
      }
      const path = this.configService.get('client.charger.getSessionPath')
      if (!path) {
        throw new InternalServerErrorException(
